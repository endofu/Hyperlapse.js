...
var Hyperlapse = function(container, params) {
	...
	var render = function() {
		if(!_is_loading && self.length()>0) {
			...
			_mesh.rotation.y = (_forward ? 0 : -Math.PI);
	 ...
	 * @param {Number} v
	 */
	this.setRotationComp = function(v) { self.rotation_comp = v; };
	 ...
	 * @returns {Boolean}
	 */
	this.forward = function() { return _forward; };
	...
	this.getNextPoint = function() {
		if(_forward) {
			if(_point_index + 1 == _h_points.length) {
				return _h_points[_h_points.length-2];
			}
			return _h_points[_point_index + 1];
		} else {
			if(_point_index == 0) {
				return _h_points[1];
			}
		...
		return _h_points[(_point_index + 1) % _h_points.length];
	};
